...
## 2026-08-26 (Part 10) — إصلاح عدم الاستجابة + توثيق النظام الجديد

### 🐛 إصلاح حرج: جمود الواجهة في المتصفح

**السبب الجذري:** الطبقتان العلويتان (palette/settings) عليهما `display:flex`
في CSS يتجاوز سمة `hidden` — فتظهران **دائماً** شفافتين ثابتتين تغطيان
الشاشة كاملة وتخطفان كل النقرات → الصفحة تبدو مجمّدة تماماً.
(اختبارات jsdom لم تكتشفها لأنها لا تحسب CSS cascade)

**الإصلاح:** قاعدة `.overlay[hidden]{display:none!important}`

### ⚡ تحسين استجابة إضافي
- render انتقائي: التبويبات والقوائم والشريط لا يُعاد بناؤها إلا عند تغير
  النطاق أو الإعدادات (dirty flags) — بدل إعادة بناء كاملة عند كل نقرة

### 📚 توثيق النظام الجديد
- FUNCTION_INDEX: قسم tool-registry (1247 عنصراً)
- INDEX.md: معرفات PLAY-001..007 لحزمة الملعب كاملة
- lint: صفر أخطاء (حُلّ no-unused-expressions)

### التحقق النهائي قبل الدفع
- ✅ typecheck صفر | ✅ 1680 اختباراً | ✅ lint صفر أخطاء
...
**1. حصر شامل [PLAY-SHELL-TOOLS-001]:**
- 57 أداة مسجلة من ~190 وظيفة فعلية (92 طريقة محركات + 69 دالة مدمجة + 32 بلوك)
- 4 مواضع ظهور: toolbar (≤9 عمل) / overflow ⋮ / context (زر أيمن) / palette (بحث)
- سياسة منع الاكتظاظ: أدوات العمل ≤9، أدوات العرض (مفاتيح اللوحات) معفاة دائماً
- **6 خانات محجوزة شبحية**: AI، تعاون، ماكرو، رسوم بيانية، PDF، سجل إصدارات — كلها تشير لمصادر جاهزة في المشروع
- searchTools للوحة الأوامر: بالاسم العربي والمعرف والتلميح

**2. الواجهة البصرية الحية [PLAY-APP-001]:**
- PlaygroundApp: تركيب DOM بالمناطق السبع (menubar/tabbar/toolbar/canvas/layers/properties/statusbar)
- تبويبات ملونة بألوان النطاقات، شريط سياقي يتبدل مع المحرر
- قوائم منسدلة حية، قائمة ⋮ فائض، لوحة أوامر بحث فوري
- صفحة إعدادات: 14 بطاقة ثيمة + 6 خلفيات — اختيار فعلي يغير الحالة
- المشاهد مربوطة بالمحركات: إدراج صورة في Writer يزيد البلوقات فعلاً، Calc يحسب الصيغ
- CSS كامل مدمج (~200 سطر) بنمط فاتح نقي RTL

**إصلاحات أثناء الربط:**
- extractHtmlAst كان يتجاهل السمات → التقاط id/class كامل
- المعاينة المخزنة لا تتحدث بعد الأوامر → إعادة بناء دائمة
- أدوات العرض كانت تدفع للفائض → إعفاء دائم (UX)

### التحقق
- ✅ typecheck صفر | ✅ **1680 اختباراً** (+22) في 103 ملف | lint صفر أخطاء

## 2026-08-26 (Part 7) — PHASE-09: قشرة الملعب التجريبي بنمط ليبرا أوفيس

### المنجزات

**حزمة @libretext/playground الجديدة [PLAY-*]:**

1. **آلة حالة القشرة [PLAY-SHELL-001]** — نقية بلا DOM وقابلة للاختبار:
   - تبويب بين المحررات الأربعة (نمط ONLYOFFICE المفاهيمي فقط — ترخيصهم AGPL فلا نسخ كود، نمط التخطيط القياسي حصراً)
   - لوحات بحالات ثلاث: ظاهرة/مخفية/مثبتة — الإخفاء يفك التثبيت، التثبيت يستلزم الظهور
   - صفحة إعدادات + ثيم نشط + خلفية كانفاس

2. **الأيقونات الرمزية [PLAY-SHELL-ICONS-001]:**
   - 18 أيقونة SVG مضمنة 16px بنمط ليبرا أوفيس
   - ألوان النطاقات القياسية: Writer أزرق #2a6099، Calc أخضر، Impress برتقالي، Base بنفسجي

3. **نموذج القوائم [PLAY-SHELL-MENU-001]:**
   - 4 قوائم لكل محرر (ملف/تحرير/عرض/إدراج) — عقود بيانات نقية
   - الأدوات المشتركة (جديد/حفظ/تراجع/إعادة) موحدة عبر shared:true
   - قائمة الإدراج سياقية: writer=معادلة/كود تفاعلي/فهرس، calc=تفقيط، impress=انتقالات، base=علاقات

4. **المشاهد المربوطة بالمحركات [PLAY-VIEWS-001]:**
   - WriterView→WriterEngine، CalcView→حساب صيغ، ImpressView→ImpressEngine (ثيم+شرائح+انتقالات)، BaseView→BaseEngine (رفض الأعمدة المكررة يعمل!)
   - كل مشاهدة تكشف handleCommand الموحد — القشرة لا تعرف التفاصيل

5. **صفحة الإعدادات [PLAY-PANELS-SETTINGS-001]:**
   - معرض ثيمات: 6 رسمية + 8 مولدة من الطيف اللانهائي = 14 بطاقة
   - 6 خلفيات كانفاس فاتحة (سحاب/وهج/نسيم/نعناع/ليلكي)

### التحقق
- ✅ typecheck صفر | ✅ 1658 اختباراً (+19) في 101 ملف | ✅ lint صفر أخطاء
