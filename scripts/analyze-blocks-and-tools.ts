...
function scanFiles(dir: string, acc: string[] = []): string[] {
  ...
  for (const e of fs.readdirSync(dir, { withFileTypes: true })) {
    ...
    if (e.isDirectory()) {
      if (!SKIP.includes(e.name)) scanFiles(full, acc);
      continue;
    }
    if (
...
function analyzeFile(filePath: string): {
  ...
  return {
    functions: countPatterns(
      content,
      /(?:export\s+)?(?:function|const\s+\w+\s*=\s*(?:\([^)]*\)\s*=>|function))\s+\w*/g,
    ),
  ...
  return [
    {
      ...
      domain: 'Writer',
      category: 'Text',
      description: 'Text block with rich formatting',
    ...
    },
    {
      ...
      domain: 'Writer',
      category: 'Text',
      description: 'Heading H1-H6 with auto-numbering',
    ...
    },
    {
      ...
      domain: 'Universal',
      category: 'Data',
      description: 'Advanced table with cell merge and formulas',
    ...
    },
    {
      ...
      domain: 'Calc',
      category: 'Data',
      description: 'Spreadsheet cell with formula support (A1, B2)',
    ...
    {
      type: 'math',
      nameAr: 'Muadala Riyadiya',
      domain: 'Universal',
      category: 'Plugin',
      description: 'LaTeX/KaTeX math equation',
    ...
    },
    {
      ...
      domain: 'Impress',
      category: 'Visual',
      description: 'Vector shape with 2D transforms',
    ...
    },
    {
      ...
      domain: 'Impress',
      category: 'Layout',
      description: 'Presentation slide with layers',
    ...
    },
    {
      ...
      domain: 'Base',
      category: 'Data',
      description: 'Database record with typed fields',
    ...
    },
    {
      ...
      domain: 'Universal',
      category: 'Visual',
      description: 'Image with safe resize and filters',
    ...
    {
      type: 'callout_box',
      nameAr: 'Sunduq Mulaahazat',
      domain: 'Writer',
      category: 'Layout',
      description: 'Callout box (info, warning, success)',
...
function buildPackageStats(): Array<{
  name: string;
  files: number;
  lines: number;
  functions: number;
...
export function main(): void {
  ...
  fs.writeFileSync(
    path.join(ROOT, 'BLOCKS_AND_TOOLS_REGISTRY.md'),
    buildRegistryMd(blocks),
    'utf-8',
  );
  ...
  fs.writeFileSync(
    path.join(ROOT, 'BLOCKS_ANALYTICS.json'),
    JSON.stringify({ generatedAt: new Date().toISOString(), analytics, blocks }, null, 2),
    'utf-8',
  );
