  ...
  public toHtml(markdown: string): string {
    ...
    text = text.replace(/```([\s\S]*?)```/g, (_m, code) => {
      ...
      return createPlaceholder(
        `<pre class="bg-slate-100 p-3 rounded-lg border border-slate-200 font-mono text-xs overflow-x-auto text-slate-800 my-2"><code>${escaped}</code></pre>`,
      );
    });
    text = text.replace(/\$\$([\s\S]*?)\$\$/g, (_m, math) =>
      createPlaceholder(latexEngine.renderToHtml(math, true)),
    );
    text = text.replace(/\$([^$\n]+)\$/g, (_m, math) =>
      createPlaceholder(latexEngine.renderToHtml(math, false)),
    );
    text = this.parseMarkdownTables(text);
    text = text.replace(/<div class="overflow-x-auto my-3">[\s\S]*?<\/div>/g, (tableHtml) =>
      createPlaceholder(tableHtml),
    );
    text = text.replace(
      /`([^`]+)`/g,
