 ...
 * ═══════════════════════════════════════════════════════════════════════════
 * 📄 الملف: unit-calc-engine.ts
 * 📂 المسار: packages/algorithms/src/computation/unit-calc-engine.ts
 * 🎯 الهدف الرئيسي: محرك حساب الوحدات والأوقات والملاحظات السحرية (Unit & Magic Notes Engine)
 * 📋 المعايير: صفر اعتماديات خارجية، محرك Recursive-Descent بحت
 * 🏷️ المعرف: ALGO-CALC-001
 ...
 *    - Magic Notes Inline Derivation: السطر المنتهي بـ `=` يُشتق جوابه في العرض فقط دون حفظه
 *    - Single Base Unit Conversion: تحويل كل عائلة وحدات عبر قاعدة واحدة لمنع التراكم
 *    - Date Tokenization: تحليل ISO Dates قبل الأرقام لمنع قراءتها كعمليات طرح
 *    - Float Dust Cleanup via `tidy()` لمنع الأخطاء العشرية الدقيقة
 * ═══════════════════════════════════════════════════════════════════════════
 * ⚠️ نقاط الخطر الإلزامية | Mandatory Gotchas:
 *    1. الأمان الصارم: حظر استخدام eval أو new Function كحاجز أمني مطلق.
 *    2. لا تخزين للجواب: القيمة المشتقة تظهر كعرض فقط (View State).
 *    3. فحص حدود المصفوفات والمؤشرات ومنع تجاوز طول التعبير (> 200 chars).
 * ═══════════════════════════════════════════════════════════════════════════
 * 🩹 البرمجة الدفاعية | Defensive Coding:
 *    - إرجاع null بأمان عند التعابير غير الصالحة دون رمي أي استثناءات.
 *    - فحص صلاحية التواريخ عبر Calendar Validation.
 *    - تنظيف الأرقام العشرية الدقيقة عبر tidy().
 * ═══════════════════════════════════════════════════════════════════════════
 * 📊 الدوال والخوارزميات | Functions & Algorithms:
 *    - evaluateCalc: تقييم تعبير حسابي واحد إلى CalcVal
 *    - answerCalc: اشتقاق جواب سطرMagic Note
 *    - parseDefinition: تحليل تعريف متغير من سطر
 *    - feedCalcLine: تغذية سطر في السياق الحسابي
 *    - buildPageCalcContext: بناء سياق حسابي من صفوف المستند
 *    - formatCalcVal: تنسيق CalcVal إلى نص مقروء
 *    - freshCalcContext: إنشاء سياق حسابي فارغ
 * ═══════════════════════════════════════════════════════════════════════════
 * 📝 ملاحظات التطوير | Development Notes:
 *    - المصدر: webpainter-next_X_7/src/shared/lib-core/computational-notebook/unit-calc-engine.ts
 *    - مُكيَّف ليتوافق مع بُنى libretext-editor-suite
 * ═══════════════════════════════════════════════════════════════════════════
 * 🔗 الملفات المرتبطة | Linked Files:
 *    - 📇 الفهرس: FUNCTION_INDEX.md
 *    - 📄 مرتبط مباشر: formula/evaluator.ts (محرك تعبيرات جداول البيانات)
 *    - 🧪 اختبارات: tests/computation/unit-calc-engine.test.ts
 * ═══════════════════════════════════════════════════════════════════════════
 * 👤 المالك: Hossam El-Din Abdel-Moaty El-Khouly - All rights reserved
 * ⚖️ الترخيص: MIT License
 * 📚 المصادر المقتبسة: webpainter-next computational-notebook (MIT)
 * ═══════════════════════════════════════════════════════════════════════════
 */
...
interface UnitDef { base: string; k: number; off?: number }
...
const toBase = (v: CalcVal): number => {
  const u = UNITS[v.u!]
  return (v.n + (u?.off ?? 0)) * (u?.k ?? 1)
}

const fromBase = (n: number, to: string): number => {
  const u = UNITS[to]
  return n / (u?.k ?? 1) - (u?.off ?? 0)
}
...
class Parser {
  ...
  add(a: CalcVal, b: CalcVal, sign: number): CalcVal | null {
    ...
    if (a.u && b.u) {
      const ua = UNITS[a.u], ub = UNITS[b.u]
...
export function parseDefinition(text: string, ctx: UnitCalcContext = {}): { name: string; val: CalcVal } | null {
  const m = DEF.exec(text)
  if (!m || asksForAnswer(text)) return null
  const name = (m[1] ?? '').trim().toLowerCase()
  if (!name || unitOf(name) || ['today', 'tomorrow', 'yesterday', 'sum', 'total', 'average'].includes(name)) {
    return null
  }
...
export function answerCalc(text: string, ctx: UnitCalcContext = {}, locale?: string): string | null {
