...
export type NotebookCellType =
  'markdown' | 'latex' | 'code' | 'callout' | 'quote' | 'canvas-drawing';
      ...
      slides: [
        {
          ...
          cells: [
            ...
            },
            {
              ...
              content:
                'يمكنك إضافة خلايا كود أو معادلات أو نصوص ماركدون مع دعم التحديث الفوري المباشر.',
              calloutType: 'info',
            },
          ],
        },
...
<div class="notebook-cell-code my-3 border border-slate-200 rounded-xl overflow-hidden shadow-xs bg-slate-900/5">
  ${
    cell.title
      ? `<div class="px-4 py-2 bg-slate-100 border-b border-slate-200 text-xs font-bold text-slate-700 flex items-center justify-between">
  ...
  ${
    cell.output
      ? `<div class="px-4 py-2.5 bg-emerald-50/70 border-t border-emerald-200 text-xs font-medium text-emerald-800 flex items-center gap-2">
    <span>▶ النتيجة:</span>
