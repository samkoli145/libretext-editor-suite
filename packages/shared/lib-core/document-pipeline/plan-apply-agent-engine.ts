...
export function validateDoc(doc: SpaceDoc, registry?: BlockRegistry): ValidateResult {
  ...
  if (!pages.length) {
    add({
      code: 'no-pages',
      severity: 'error',
      message: 'This space has no pages.',
  ...
  for (const p of pages) {
    ...
    for (const b of Array.isArray(p.blocks) ? p.blocks : [])
      claim(b.id, { page: p.id, block: b.id }, 'block');
  }
      ...
      if (b.type === 'prop') {
        ...
        if (!key) {
          add({
            ...
            path: 'key',
            message:
              'A prop block has no `key`, so it names no field: nothing reads it, the header strip cannot label it, and a board cannot group by it.',
            fix: `Set key to one of: ${[...fieldOf.keys()].join(', ')}.`,
          });
        } else if (!f) {
          add({
            ...at,
            code: 'unknown-field-key',
            severity: 'info',
            path: 'key',
      ...
      if (b.type === 'image') {
        ...
        if (!String(b.alt ?? '').trim()) {
          add({
            ...
            path: 'alt',
            message:
              'This image has no alt text — screen readers get nothing, and alt is what a reader SEES if the image is remote and unloaded.',
...
export function outlineDoc(doc: SpaceDoc): OutlineResult {
  ...
  const node = (p: Page, depth: number): OutlineNode => {
    ...
    for (const b of p.blocks) {
      ...
      if (b.type === 'pagelink' && typeof b.page === 'string' && !links.includes(b.page))
        links.push(b.page);
...
export function statsDoc(doc: SpaceDoc): StatsResult {
  ...
  for (const [key, value] of Object.entries(assets)) {
    ...
    each.push({
      key,
      bytes,
      mime: /^data:([^;,]+)/.exec(value)?.[1] ?? 'unknown',
      used: use.get(key) ?? 0,
    });
  }
...
const fail = (err: string, detail?: string): PlanError => ({
  ok: false,
  err,
  ...(detail ? { detail } : {}),
});
...
function splitPatch(
...
): { sets: Record<string, unknown>; dels: string[] } | PlanError {
  ...
  for (const [k, v] of Object.entries(patch)) {
    ...
    if (!jsonSafe(v)) {
      return fail(
        'not-serializable',
        `"${k}" holds something JSON cannot carry (a function, a DOM node, a Date, a cycle). It would vanish or throw at save time.`,
      );
    }
    sets[k] = v;
  }
...
export function planInsertBlocks(
  ...
  for (const raw of blocks) {
    if (raw != null && !jsonSafe(raw))
      return fail('not-serializable', 'a block holds something JSON cannot carry');
  ...
  return {
    ...
    apply() {
      const at =
        afterId == null ? page.blocks.length : page.blocks.findIndex((b) => b.id === afterId) + 1;
...
export function planUpdateBlock(
  doc: SpaceDoc,
  id: string,
  patch: unknown,
  registry?: BlockRegistry,
): Plan<{ id: string; page: string }> {
  ...
  if ('id' in patch)
    return fail('immutable', 'a block id never changes — links, backlinks and undo key on it');

  const { block, page } = at;
  const type = 'type' in patch ? patch.type : block.type;
  if (typeof type !== 'string' || !type)
    return fail('bad-patch', 'type must be a non-empty string');
  ...
  if ('parent' in patch && patch.parent != null) {
    ...
    if (!page.blocks.some((b) => b.id === owner))
      return fail('no-such-block', `parent "${owner}" is not a block on page "${page.id}"`);
    if (descendants(page, id).has(owner))
      return fail('cycle', `parent "${owner}" is nested inside "${id}"`);
  }

  const split = splitPatch(patch);
  if ('ok' in split) return split;
  const { sets, dels } = split;
  if (dels.includes('type'))
    return fail('bad-patch', 'type cannot be deleted; set it to another type instead');

  if (typeof sets.html === 'string') sets.html = normalizeHtml(sets.html, type);
  else if (
    sets.type &&
    typeof block.html === 'string' &&
    (sets.type === 'code') !== (block.type === 'code')
  ) {
...
export function planRemoveBlocks(
  doc: SpaceDoc,
  ids: unknown,
): Plan<{ removed: string[]; missing: string[]; added: string[] }> {
...
export function planMoveBlock(
  ...
  id: string,
  to: unknown,
): Plan<{ id: string; pageId: string; moved: string[] }> {
  if (to !== undefined && !isPatch(to))
    return fail('bad-patch', 'pass { pageId?, afterId?, beforeId?, parent? }');
  ...
  if (anchorId != null) {
    if (kin.has(String(anchorId)) || String(anchorId) === id)
      return fail('cycle', 'a block cannot be moved inside its own subtree');
    if (!dest.blocks.some((b) => b.id === anchorId))
      return fail('no-such-block', `"${anchorId}" is not a block on page "${dest.id}"`);
  }

  let parent: string | null | undefined = spec.parent;
  if (parent != null) {
    const owner = String(parent);
    if (owner === id || kin.has(owner))
      return fail('cycle', 'a block cannot be nested under itself');
    if (!dest.blocks.some((b) => b.id === owner))
      return fail('no-such-block', `parent "${owner}" is not a block on page "${dest.id}"`);
  } else if (parent === undefined) {
    parent =
      at.block.parent && dest.blocks.some((b) => b.id === at.block.parent) ? at.block.parent : null;
  }
...
export function planUpdatePage(doc: SpaceDoc, id: string, patch: unknown): Plan<{ id: string }> {
  ...
  if ('id' in patch)
    return fail('immutable', 'a page id never changes — every #p/ link and backlink keys on it');
  if ('blocks' in patch)
    return fail(
      'immutable',
      'use insertBlocks / updateBlock / removeBlocks / moveBlock for block structure',
    );
  ...
  if ('parent' in sets) {
    ...
    const seen = new Set<string>();
    for (
      let up: string | undefined = owner;
      up && !seen.has(up);
      up = doc.pages.find((p) => p.id === up)?.parent
    ) {
      seen.add(up);
...
export function planRemovePage(
  doc: SpaceDoc,
  id: string,
  opts: RemovePageOpts = {},
): Plan<{ removed: string[]; rehomed: number; links: number }> {
  const page = doc.pages.find((p) => p.id === id);
  if (!page) return fail('no-such-page', id);
  ...
  if (doomed.size >= doc.pages.length)
    return fail('last-page', 'that would remove every page in the space');
...
const noSuchField = (k: string, schema: FieldSpec[]): PlanError =>
  fail(
    'no-such-field',
    `"${k}" is not a field in this space. The schema is doc.fields, which declares: ${schema.map((x) => x.key).join(', ')}. Add the field there first — a value whose field nothing declares cannot be labelled, edited or grouped by.`,
  );
...
export function planSetField(
  ...
  if (value !== null && value !== undefined && !jsonSafe(value)) {
    return fail(
      'not-serializable',
      `the value for "${k}" holds something JSON cannot carry (a function, a DOM node, a Date, a cycle). It would vanish or throw at save time.`,
    );
  }
...
export function issuesReport(doc: SpaceDoc, query: IssueQuery = {}): IssueReport[] {
  ...
  for (const page of doc.pages) {
    ...
    const group =
      raw === undefined || raw === null || raw === '' ? undefined : opt ? opt.group : 'unknown';
    ...
    if (want && !want.some((g) => (g === 'open' ? !(group && FINISHED.has(group)) : g === group)))
      continue;
...
export function planNewIssue(doc: SpaceDoc, spec: unknown): Plan<NewIssueResult> {
  if (spec !== undefined && !isPatch(spec))
    return fail('bad-patch', 'pass { title?, parent?, ...fieldValues }');
  ...
  if (parent && !doc.pages.some((p) => p.id === parent))
    return fail('no-such-page', `parent "${parent}"`);
  ...
  for (const [k, v] of Object.entries(src)) {
    ...
    const f = schema.find((x) => x.key === k);
    if (!f) return noSuchField(k, schema);
    if (v !== null && v !== undefined && !jsonSafe(v))
      return fail('not-serializable', `the value for "${k}" holds something JSON cannot carry`);
    values[k] = v ?? '';
  ...
  const props = seed.map((f) =>
    propBlock(f, f.key in values ? values[f.key] : (f.def ?? ''), uid('b')),
  );
