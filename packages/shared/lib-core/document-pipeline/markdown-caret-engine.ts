...
export class MarkdownCaretEngine {
  ...
  private readonly rules: InlineFormatRule[] = [
    ...
    {
      pattern: /`([^`]+)`/g,
      replace: (_, code) => `<code class="bg-gray-100 text-gray-800 px-1 py-0.5 rounded font-mono text-xs border border-gray-200">${code}</code>`,
      label: 'code',
    },
    // مشطوب: ~~strike~~
    {
    ...
    {
      pattern: /==([^=]+)==/g,
      replace: (_, text) => `<mark class="bg-amber-100 text-amber-900 px-1 py-0.2 rounded border border-amber-200">${text}</mark>`,
      label: 'highlight',
    },
  ]
  ...
  markdownToHtml(markdown: string): string {
    ...
    let html = markdown
      ...
      .replace(/^# (.*$)/gim, '<h1 class="text-xl font-extrabold text-gray-900 mt-5 mb-2.5">$1</h1>')
      // اقتباس
      .replace(/^\> (.*$)/gim, '<blockquote class="border-r-4 border-blue-400 pr-3 py-1 my-2 bg-blue-50/50 text-gray-700 italic">$1</blockquote>')
      // قوائم نقطية
