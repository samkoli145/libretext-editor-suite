...
export class LinkedChartBridge {
  ...
  static deriveFromTableGrid(
    ...
    for (let r = rowStart; r < tableData.length; r++) {
      ...
      const label =
        hasHeaderColumn && row && row[0] !== undefined
          ? String(row[0])
          : `عنصر ${r - rowStart + 1}`;
    ...
    for (let c = colStart; c < numCols; c++) {
      const seriesName =
        hasHeaderRow && tableData[0] && tableData[0][c] !== undefined
    ...
    if (seriesList.length === 0) {
      return {
        ...
        error: 'لم يتم العثور على أعمدة رقمية صالحة',
      };
    }
   ...
   */
  static deriveFromScratchpadVars(
    variables: Array<{ name: string; value: unknown; description?: string }>,
  ): DerivedChartData {
    ...
    if (numericVars.length === 0) {
      return {
        ...
        series: [],
        hasPercentages: false,
        error: 'لا توجد متغيرات رقمية صالحة',
      };
    }
