...
export class MouseInteractionTrinity {
  ...
  public static startDragSession(
    ...
    const { startPoint, initialBounds, enableAxisLock = true, enableCloneDrag = true, onMove, onCommit, onCancel } = options
...
export function performDeepSelect(
  point: Point2D,
  elements: Array<{ id: string; x: number; y: number; width: number; height: number; zIndex?: number; locked?: boolean }>,
