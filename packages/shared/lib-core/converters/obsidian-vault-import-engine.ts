...
export function inlineHtml(src: string): string {
  ...
  s = s.replace(
    /<((?:https?|mailto):[^>\s]+)>/gi,
    (_m, url: string) => hold(`<a href="${esc(url)}">`) + url + hold('</a>'),
  );
  ...
  s = s.replace(/!?\[\[([^\]]+)\]\]/g, (_m, inner: string) => {
    ...
    return (
      hold(`<a href="${WIKI_SCHEME}${encodeURIComponent(target.trim())}">`) +
      (alias ?? target).trim() +
      hold('</a>')
    );
  });
  ...
  s = s.replace(
    ...
    (m: string, text: string, url: string) =>
      /^(https?:|mailto:)/i.test(url) ? hold(`<a href="${esc(url)}">`) + text + hold('</a>') : m,
  );
  ...
  s = s.replace(
    /\*\*(?=\S)([\s\S]+?)\*\*/g,
    (_m, x: string) => hold('<strong>') + x + hold('</strong>'),
  );
  s = s.replace(
    /(^|[^\w\\])__(?=\S)([\s\S]+?)__(?!\w)/g,
  ...
  for (; i < lines.length; i++) {
    ...
    if (fence) {
      ...
      add(
        mk('code', {
          html: esc(buf.join('\n')),
          ...(fence[2] ? { lang: fence[2].toLowerCase() } : {}),
        }),
        owner,
      );
      continue;
...
export function planImport(
  ...
  const { index, collisions } = linkIndex(
    src,
    parsed,
    filePage,
    tops.size === 1 ? [...tops][0] : '',
  ...
  for (const page of pages) {
    for (const b of page.blocks) {
      ...
      const r = resolveWikilinks(
        b.html,
        (target) => index.get(linkKey(target)) ?? opts.resolveExisting?.(linkKey(target)),
      );
      b.html = r.html;
...
export function resolveWikilinks(
  html: string,
  lookup: (target: string) => string | undefined,
): Resolution {
  let linked = 0;
  let dangling = 0;
