...
export function isScratchpadError(val: unknown): val is ScratchpadError {
  return val instanceof ScratchpadError || (typeof val === 'object' && val !== null && (val as any).isError === true)
}
...
export const ERR_VALUE = (msg = 'قيمة غير صالحة') =>
  new ScratchpadError('#VALUE!', msg)

export const ERR_DIV0 = () =>
  new ScratchpadError('#DIV/0!', 'القسمة على صفر غير مسموحة')
...
export function normalizeVar(raw: Record<string, unknown>): ScratchpadVar {
  ...
  const id = typeof cleanRaw.id === 'string' ? cleanRaw.id : `var_${Date.now().toString(36)}_${Math.random().toString(36).slice(2, 7)}`
