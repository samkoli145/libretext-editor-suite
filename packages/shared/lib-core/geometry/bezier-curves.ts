...
export function evalCubicDerivative(p0: Pt, cp1: Pt, cp2: Pt, p1: Pt, t: number): Pt {
  ...
  return {
    x: 3 * u * u * (cp1.x - p0.x) + 6 * u * clampedT * (cp2.x - cp1.x) + 3 * clampedT * clampedT * (p1.x - cp2.x),
    y: 3 * u * u * (cp1.y - p0.y) + 6 * u * clampedT * (cp2.y - cp1.y) + 3 * clampedT * clampedT * (p1.y - cp2.y),
  }
}
 ...
 */
export function splitCubicSegment(seg: BezSegment, t: number): { left: BezSegment; right: BezSegment } {
  const clampedT = clamp(t, 0, 1)
  ...
  const pSplit = { x: p012.x + (p123.x - p012.x) * clampedT, y: p012.y + (p123.y - p012.y) * clampedT }
 ...
 */
export function nearestTOnCubic(seg: BezSegment, target: Pt, samples: number = 24): { t: number; point: Pt; distance: number } {
  let bestT = 0
 ...
 */
export function mirrorHandle(node: BezNode, handleMoved: 'in' | 'out', keepLength: boolean = false): BezNode {
